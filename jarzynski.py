...
if len(have) > 0:
    x, jar_free_ener = exp_average( have )
    jar_force        = dWdx( x, jar_free_ener )
    jf_smooth        = smooth_f(jar_force, 1000 )
    jf_smooth2       = smooth_f(jar_force, 10000 )
    try:
        jar_dat = np.hstack( (x, jar_force, jar_free_ener, jf_smooth, jf_smooth2) )
    except:
        print("failed hstack:")
        print(np.shape(x))
        print(np.shape(jar_force))
        print(np.shape(jar_free_ener))
    np.savetxt("%s_jarzinski.dat" % stem, jar_dat)
else:
    print("had only files: "+str(have))
